...
const mines = new Map(), minesSeen = new Set();
const stalkers = new Map(), stalkersSeen = new Set();
const REGROW_S = 120;
...
function updateTreeField(px, pz) {
  ...
  for (let dx = minC; dx <= maxC; dx++) {
    for (let dz = minC; dz <= maxC; dz++) {
      ...
      const inRange = Math.hypot(wx - px, wz - pz) < SPAWN_R;
      if (inRange && !trees.has(key)) spawnTree(cx, cz, key);
      if (inRange && !mines.has(key) && !minesSeen.has(key)) {
        spawnMine(cx, cz, key);
        minesSeen.add(key);
      }
      if (inRange && !stalkers.has(key) && !stalkersSeen.has(key)) {
        spawnStalker(cx, cz, key);
        stalkersSeen.add(key);
      }
    }
  }
  ...
  for (const [key, m] of mines) {
    if (Math.hypot(m.x - px, m.z - pz) > SPAWN_R + 8) {
      scene.remove(m.mesh);
      mines.delete(key);
      minesSeen.delete(key);
    }
  }
  for (const [key, s] of stalkers) {
    if (Math.hypot(s.x - px, s.z - pz) > SPAWN_R + 20) {
      scene.remove(s.group);
      stalkers.delete(key);
      stalkersSeen.delete(key);
    }
  }
}
...
const input = { x: 0, z: 0, action: false };
{
  ...
  $("btn-pulse").addEventListener("pointerdown", (e) => { e.preventDefault(); firePulse(); });
  window.addEventListener("keydown", (e) => { if (e.code === "KeyF") firePulse(); });
}
...
window.TELEPORT = (x, z) => { player.position.set(x, 0, z); updateTreeField(x, z); };
window.APPLY = applyLevels;
window.DIAG2 = () => ({ trees: trees.size, mines: mines.size, stalkers: stalkers.size, tier: currentTier });
window.DIAG = () => {
...
function fellTree(tree) {
  ...
  if (pet && tree.fruits.length && Math.random() < 0.25) {
    // The glowbug shakes a bonus fruit loose.
    const extra = tree.fruits[0].clone();
    extra.position.copy(tree.fruits[0].position);
    tree.group.add(extra);
    launchFruit(extra, tree);
...
function damagePlayer(amount) {
  ...
  camShake = Math.min(0.5, camShake + 0.25);
}

// ---------- Spore mines ----------
const mineGeo = new THREE.SphereGeometry(0.55, 10, 8);
const mineMat = new THREE.MeshLambertMaterial({ color: 0x7a3aa8, emissive: 0xb040ff, emissiveIntensity: 0.5 });

function spawnMine(cx, cz, key) {
  // Mines live in cells without trees; density grows with distance.
  if (treeAtCell(cx, cz)) return;
  const x = cx * CELL + CELL / 2 + (hash2(cx, cz, 30) - 0.5) * 4;
  const z = cz * CELL + CELL / 2 + (hash2(cx, cz, 31) - 0.5) * 4;
  const r = Math.hypot(x, z);
  if (r < CLEAR_R + 4) return;
  const tier = tierAt(r);
  const density = 0.05 + tier * 0.03;
  if (hash2(cx, cz, 32) > density) return;
  const mesh = new THREE.Mesh(mineGeo, mineMat.clone());
  mesh.scale.y = 0.55;
  mesh.position.set(x, 0.28, z);
  scene.add(mesh);
  mines.set(key, { mesh, x, z, tier, t: hash2(cx, cz, 33) * 10 });
}

function explodeMine(key, m, hurts) {
  mines.delete(key);
  scene.remove(m.mesh);
  for (let i = 0; i < 14; i++) spawnSpark(m.x, 0.6, m.z, 0xb040ff);
  const d = Math.hypot(player.position.x - m.x, player.position.z - m.z);
  if (hurts && d < 3.2) {
    damagePlayer((16 + m.tier * 8) * (1 - d / 3.6));
    showMsg("Spore mine! The suit filters taste like burnt raisins.", 2600);
  }
}

function updateMines(t) {
  for (const [key, m] of mines) {
    m.mesh.scale.setScalar(1 + Math.sin(t * 3 + m.t) * 0.1);
    m.mesh.scale.y *= 0.55;
    const d = Math.hypot(player.position.x - m.x, player.position.z - m.z);
    if (d < 1.25) explodeMine(key, m, true);
  }
}

// ---------- Stalkers ----------
const stalkerBodyGeo = new THREE.IcosahedronGeometry(0.55, 0);
const stalkerEyeGeo = new THREE.SphereGeometry(0.09, 6, 6);

function spawnStalker(cx, cz, key) {
  const x = cx * CELL + CELL / 2 + (hash2(cx, cz, 40) - 0.5) * 4;
  const z = cz * CELL + CELL / 2 + (hash2(cx, cz, 41) - 0.5) * 4;
  const r = Math.hypot(x, z);
  if (r < 38) return; // inner belt is safe
  const tier = tierAt(r);
  if (hash2(cx, cz, 42) > 0.022 + tier * 0.012) return;
  let count = 0;
  for (const s of stalkers.values()) count++;
  if (count >= 4) return;
  const group = new THREE.Group();
  const body = new THREE.Mesh(
    stalkerBodyGeo,
    new THREE.MeshLambertMaterial({ color: 0x2a1a3a, emissive: 0x5a2080, emissiveIntensity: 0.35 })
  );
  ...
  for (const side of [-1, 1]) {
    const eye = new THREE.Mesh(
      stalkerEyeGeo,
      new THREE.MeshLambertMaterial({ color: 0xffffff, emissive: 0xffd040, emissiveIntensity: 1.4 })
    );
    eye.position.set(side * 0.2, 0.75, 0.42);
    group.add(eye);
  }
  group.position.set(x, 0, z);
  scene.add(group);
  stalkers.set(key, { group, x, z, tier, key, mode: "idle", stun: 0, biteCd: 0, wanderA: hash2(cx, cz, 43) * 6.28 });
}

function updateStalkers(dt, t) {
  for (const [key, s] of stalkers) {
    const px = player.position.x, pz = player.position.z;
    const d = Math.hypot(px - s.x, pz - s.z);
    if (s.stun > 0) {
      s.stun -= dt;
      s.group.rotation.y += dt * 9;
      continue;
    }
    s.biteCd = Math.max(0, s.biteCd - dt);
    let speed = 0;
    let tx = 0, tz = 0;
    const playerSafe = Math.hypot(px, pz) < CLEAR_R + 2;
    if (d < 13 && !playerSafe && s.biteCd <= 0) {
      s.mode = "chase";
      tx = px - s.x; tz = pz - s.z;
      speed = 2.6 + s.tier * 0.35;
    } else {
      s.mode = "idle";
      s.wanderA += dt * 0.4;
      tx = Math.cos(s.wanderA); tz = Math.sin(s.wanderA);
      speed = 0.7;
    }
    ...
    s.z += (tz / len) * speed * dt;
    // hop
    s.group.position.set(s.x, Math.abs(Math.sin(t * 6 + s.wanderA)) * 0.22, s.z);
    s.group.lookAt(px, 0.4, pz);
    if (d < 1.1 && s.biteCd <= 0) {
      damagePlayer(14 + s.tier * 7);
      showMsg("A stalker got a bite in! It seemed pleased.", 2600);
      s.biteCd = 3.2;
      // retreat hop
      s.x -= ((px - s.x) / (d || 1)) * 3;
      s.z -= ((pz - s.z) / (d || 1)) * 3;
...
function firePulse() {
  ...
  ring.position.set(player.position.x, 0.35, player.position.z);
  scene.add(ring);
  pulseRings.push({ mesh: ring, t: 0 });
  const range = 4.2 + state.pulser * 0.9;
  for (const [key, m] of [...mines]) {
    if (Math.hypot(m.x - player.position.x, m.z - player.position.z) < range) explodeMine(key, m, false);
  }
  for (const s of stalkers.values()) {
    const d = Math.hypot(s.x - player.position.x, s.z - player.position.z);
    if (d < range + 1.5) {
      s.stun = 2.2 + state.pulser * 0.5;
      const kx = (s.x - player.position.x) / (d || 1), kz = (s.z - player.position.z) / (d || 1);
      s.x += kx * (5 + state.pulser);
      s.z += kz * (5 + state.pulser);
      if (state.pulser >= 3 && d < range * 0.6) {
        // Overcharged pulse pops stalkers into credits.
        scene.remove(s.group);
        stalkers.delete(s.key);
        const goo = 25 + s.tier * 15;
        state.money += goo;
        saveGame();
        showMsg(`Stalker dissolved into ₡${goo} of exotic goo. Don't ask.`, 2600);
      }
    }
  }
}
function updatePulses(dt) {
  ...
  const btn = $("btn-pulse");
  btn.classList.toggle("hidden", !state.pulser || state.mode !== "play");
  btn.classList.toggle("cooling", pulseCd > 0);
...
// ---------- Treasures ----------
function rollTreasure(tree) {
  const [cx, cz] = tree.key.split(",").map(Number);
  const roll = hash2(cx, cz, 50);
  if (roll < 0.07) {
...
// ---------- Pet: the glowbug ----------
let pet = null;
function hatchPet(x, z) {
  const group = new THREE.Group();
  const body = new THREE.Mesh(
    new THREE.SphereGeometry(0.22, 10, 8),
    new THREE.MeshLambertMaterial({ color: 0x40e8a0, emissive: 0x40e8a0, emissiveIntensity: 0.9 })
  );
  body.position.y = 0.5;
  group.add(body);
  const glow = new THREE.Sprite(
    new THREE.SpriteMaterial({ map: glowTex, color: 0x60ffc0, transparent: true, opacity: 0.6, depthWrite: false })
  );
  ...
  scene.add(group);
  pet = { group, x, z };
  showMsg("Something hatched from that tree… and it likes you. A glowbug joins you!", 3600);
}
function updatePet(dt, t) {
  if (!pet) return;
  const tx = player.position.x - 1.1, tz = player.position.z - 1.1;
...
// ---------- Zones ----------
let currentTier = 0, zoneWarnCd = 0;
const ZONE_FOG = [0x0a0518, 0x0d0522, 0x14081e, 0x260a08, 0xbcc8d8];
const zoneFogColor = new THREE.Color(ZONE_FOG[0]);
function updateZones(dt) {
  const r = Math.hypot(player.position.x, player.position.z);
  const tier = tierAt(r);
  if (tier !== currentTier) {
    currentTier = tier;
    const need = tier === 3 ? 1 : tier === 4 ? 2 : 0;
    if (need > state.envUnit) {
      showMsg(`— ${TIERS[tier].name} — hostile atmosphere! Env Unit L${need} required. Suit venting charge!`, 3800);
    } else {
      showMsg(`— ${TIERS[tier].name} —`, 2600);
    }
  }
  const need = tier === 3 ? 1 : tier === 4 ? 2 : 0;
  if (need > state.envUnit) {
    ...
    if (zoneWarnCd <= 0) {
      zoneWarnCd = 6;
      showMsg("⚠ Suit venting charge — turn back or upgrade the Env Unit!", 2400);
    }
  }
  zoneFogColor.lerp(new THREE.Color(ZONE_FOG[tier]), Math.min(1, dt * 1.5));
  scene.fog.color.copy(zoneFogColor);
  scene.background.copy(zoneFogColor);
  if (tier === 4) { scene.fog.near = 14; scene.fog.far = 38; }
  else { scene.fog.near = 26; scene.fog.far = 54; }
}
